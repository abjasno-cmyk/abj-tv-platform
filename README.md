...
- `GET /api/nazory/translate` — chráněný cron endpoint pro zpětné i průběžné
  doplňování překladů; bez parametru bere větší backfill dávku, podporuje
  `?limit=200` a volitelně `?force=1`
...
- volitelné env proměnné: `VEROX_OPINION_TRANSLATION_MODEL`,
  ...
  `VEROX_OPINION_TRANSLATION_BATCH_SIZE`,
  `VEROX_OPINION_VISIBLE_TRANSLATION_BATCH_SIZE`
