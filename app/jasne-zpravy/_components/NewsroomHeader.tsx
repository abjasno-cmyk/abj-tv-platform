...
export function NewsroomHeader({ currentEdition, itemCount, timeToNextEditionLabel }: NewsroomHeaderProps) {
  return (
    <header className="rounded-3xl border border-[#F37021]/20 bg-gradient-to-b from-[#fffaf3] via-[#fffdfa] to-white p-6 shadow-[0_12px_30px_rgba(17,17,17,0.06)] md:p-8">
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-[#F37021]/20 bg-[#F37021]/10 px-3 py-1 text-xs font-semibold text-[#B04A00]">
              {itemCount} zpráv
            </span>
          </div>
        </div>
        <div className="flex flex-wrap gap-2 lg:justify-end">
          <Link
            ...
            className="inline-flex min-h-10 items-center rounded-lg bg-[#F37021] px-4 py-2 text-sm font-bold text-white hover:bg-[#e45f00]"
          >
            Číst aktuální vydání
          </Link>
          <Link
            href="/jasne-zpravy/archiv"
            className="inline-flex min-h-10 items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:border-[#F37021]/40 hover:text-[#F37021]"
          >
            Archiv
          </Link>
        </div>
