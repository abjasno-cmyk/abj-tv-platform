...
function toItems(feed: Awaited<ReturnType<typeof fetchFeed>>): FeedItem[] {
  if (!feed) return [];
...
export default async function AbjXPage() {
  let items: FeedItem[] = [];

  try {
    const feed = await fetchFeed({ page: 1, per_page: 80 });
    items = toItems(feed);
  } catch (error) {
    console.error("abj-x-feed-load-failed", error);
  }
