...
function VideoCardBase({
  ...
  const wallHref = `/zed?video_id=${encodeURIComponent(videoId)}&video_title=${encodeURIComponent(title)}`;
  const publishedLabel = formatPublishedLabel(publishedAt);
  const isHero = featured;
  return (
    <article className="overflow-hidden rounded-xl border border-[var(--abj-gold-dim)] bg-abj-panel shadow-[0_4px_18px_rgba(0,0,0,0.35)]">
      <Link
        href={href}
        className="group block transition-all duration-200 hover:scale-[1.02] hover:shadow-[0_12px_28px_rgba(0,0,0,0.55)]"
      >
      ...
      <div className="border-t border-[var(--abj-gold-dim)] px-3 py-2">
        <Link
          href={wallHref}
