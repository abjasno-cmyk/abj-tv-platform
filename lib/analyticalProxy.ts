...
function resolveAnalyticalBaseUrl(): string | null {
  return (
    sanitizeEnvValue(process.env.NEXT_PUBLIC_ANALYTICAL_SERVICE_URL) ??
    sanitizeEnvValue(process.env.ANALYTICAL_SERVICE_URL) ??
    null
  );
}
...
async function proxyAnalyticalRequest(request: Request, upstreamPath: string, method: "GET" | "POST"): Promise<Response> {
  ...
  if (baseCandidates.length === 0) {
    return Response.json(
      {
        error: "Missing ANALYTICAL_SERVICE_URL/NEXT_PUBLIC_ANALYTICAL_SERVICE_URL in environment.",
      },
      { status: 500 }
    );
  }
  ...
  for (const base of baseCandidates) {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        headers.set("X-Analytical-Upstream", upstreamUrl.toString());
        headers.set("X-Analytical-Upstream-Trace", attempts.join(" | "));
      }
      ...
      console.error("analytical-proxy-request-failed", { upstream: upstreamUrl.toString(), method, error });
    }
  }

  return Response.json(
    {
      error: "Analytical upstream request failed.",
      attempts,
    },
    { status: 502 }
  );
...
export async function proxyAnalyticalGet(request: Request, upstreamPath: string): Promise<Response> {
  return proxyAnalyticalRequest(request, upstreamPath, "GET");
}

export async function proxyAnalyticalPost(request: Request, upstreamPath: string): Promise<Response> {
  return proxyAnalyticalRequest(request, upstreamPath, "POST");
}
